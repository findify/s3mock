...
class GetObjectTest extends S3MockTest {
  ...
  override def behaviour(fixture: => Fixture) = {
    ...
    it should "get objects via alpakka" in {
      ...
      val result = Await.result(fixture.alpakka.download("alpakka1", "test1").runWith(Sink.seq), 1.second)
    ...
    it should "get by range" in {
      ...
      val result = Await.result(fixture.alpakka.download("alpakka2", "test2", ByteRange(1, 4)).runWith(Sink.seq), 1.second)
      val str = result.fold(ByteString(""))(_ ++ _).utf8String
      str shouldBe "ooba"
    }
  }
